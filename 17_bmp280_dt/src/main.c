...
// Obtemos a instância via device tree
#define BMP280_NODE DT_NODELABEL(bmp280)
static const struct i2c_dt_spec bmp280 = I2C_DT_SPEC_GET(BMP280_NODE);
...
int bmp280_read_calib(void)
{
    uint8_t buf[6];
    int ret = i2c_burst_read_dt(&bmp280, BMP280_REG_CALIB, buf, 6);
    if (ret < 0) return ret;
...
int main(void)
{
    ...
    uint8_t id = 0;
    if (i2c_reg_read_byte_dt(&bmp280, BMP280_REG_ID, &id) < 0 || id != 0x58) {
    ...
    i2c_reg_write_byte_dt(&bmp280, 0xF4, 0x27); // Normal mode

    if (bmp280_read_calib() < 0) {
    ...
    while (1) {
        uint8_t data[3];
        if (i2c_burst_read_dt(&bmp280, BMP280_REG_TEMP_MSB, data, 3) < 0) {
